...
Assume that all JPG images were manually copied to the same folder. For instance,
moving the files in the two orignal folders HAM10000_images_part_1 and
HAM10000_images_part_2 into a unique folder.
'''
...
def split_according_to_lesion(df, test_size_fraction=0.2):
    ...
    train_df, test_df = train_test_split(grouped, test_size=test_size_fraction, random_state=42)
...
all_ham_data_df['image_name'] = all_ham_data_df['image_name'].apply(lambda x: x + '.jpg')
