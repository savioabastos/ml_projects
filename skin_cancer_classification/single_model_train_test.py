...
if __name__ == '__main__':
    ...
    #required arguments    
    parser.add_argument('--num_desired_train_examples',type=int,help='Desired number of training examples (for both classes 0 and 1)',required=False,default=-1)
    ...
    output_dir = os.path.join('../outputs/train_test/id_' + str(simulation_ID), base_name)        
    ...
    metric_mode = 'max'
    early_stopping = EarlyStopping(monitor=metric_to_monitor, patience=10, mode=metric_mode, restore_best_weights=True)
    ...
    if False:
        ...
        reduce_lr = ReduceLROnPlateau(monitor=metric_to_monitor, factor=0.5, patience=3, verbose=1, min_delta=1e-5, mode=metric_mode)
    else:
